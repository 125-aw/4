...
class Game {
    ...
    syncPlayerInput() {
        ...
        if (this.touch.active) {
            ...
            if (ax > 0.3 || ay > 0.3) {
                this.player.input.up = this.touch.dy < -0.3;
                this.player.input.down = this.touch.dy > 0.3;
                this.player.input.left = this.touch.dx < -0.3;
                this.player.input.right = this.touch.dx > 0.3;
            }
        }
        // 清零边沿触发标记
        this._pressedOnce = {};
    ...
    applySettingsFromUI(done) {
        ...
        if (sel) this.settings.quality = sel.dataset.quality;
        const sToggle = document.getElementById('set-sound-toggle');
        const mToggle = document.getElementById('set-music-toggle');
    ...
    bindTouchControls() {
        ...
        const radius = 46; // 摇杆活动半径

        const startStick = (clientX, clientY) => {
            this.touch.active = true;
            stick.classList.add('active');
            this._stickCenter = stickRect();
            this._stickId = true;
        ...
        stick.addEventListener('touchend', (e) => { e.preventDefault(); endStick(); }, { passive: false });

        // 鼠标也支持（桌面调试）
        let mouseStick = false;
        stick.addEventListener('mousedown', (e) => { mouseStick = true; startStick(e.clientX, e.clientY); });
        window.addEventListener('mousemove', (e) => { if (mouseStick) moveStick(e.clientX, e.clientY); });
        window.addEventListener('mouseup', () => { if (mouseStick) { mouseStick = false; endStick(); } });
